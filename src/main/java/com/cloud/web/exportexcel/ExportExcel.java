...
public class ExportExcel {
    ...
    public void getValue(List<WorkSheetDetail> userList, FileOutputStream fout) {
        try {
            ...
            //金额
            CellRangeAddress callRangeAddressnumber1 = new CellRangeAddress(userList.size() + 5, userList.size() + 5,
                    0, 2);//起始行,结束行,起始列,结束列
            CellRangeAddress callRangeAddressnumber2 = new CellRangeAddress(userList.size() + 5, userList.size() + 5,
                    3, 7);//起始行,结束行,起始列,结束列
